            ...
            var _this = this,
                ...
                exec = {
                    prev: function() {
                        ...
                        var delArr = JSON.parse(localStorage.getItem("exceptList"));


                        $(".days").find(".nima").each(function(){
                            $(this).removeClass('nima');
                        });
                        $(".days").find(".hehe").each(function(){
                            var x=y+''+(m>9?m:("0"+m))+''+($(this).text()>9?$(this).text():("0"+$(this).text()));
                            for(var i=0; i< list.length;i++){
                                var s=list[i]["TASKTIME"].replace(new RegExp(/(\-)/g),"");
                                var regEx = new RegExp("\\-","gi");
                                var dependedVal=list[i]["TASKTIME"].replace(regEx,"/");
                                var taskData = new Date(dependedVal);
                                // s=s.replace(new RegExp(/(\-)/g),"");
                                console.log("hehe:"+s+"ssssssssssss"+x);
                                if(list[i]["REPEATTERM"]=='0'){
                                    if(x==s){
                                        $(this).addClass("nima");
                                    }
                                }else {
                                    ...
                                    while(x>=s){
                                        var dd = taskData.getDate() < 10 ? "0" + taskData.getDate() : taskData.getDate().toString();
                                        var mm = taskData.getMonth()+1;
                                        var yyyy = taskData.getFullYear().toString(); //2011
                                        s=yyyy+""+(mm>9?mm:('0'+mm))+""+dd;

                                        if((!delArr || !delArr[s] || !delArr[s][list[i]["ID"]]) && x==s){
                                            $(this).addClass("nima");
                                        }
                                        if(list[i]["REPEATTERM"]=='4'){
                                            taskData.setFullYear(taskData.getFullYear()+1);
                                        }else if(list[i]["REPEATTERM"]=='3'){
                                            taskData.setMonth(taskData.getMonth()+1);
                                        }else if(list[i]["REPEATTERM"]=='2'){
                                            taskData.setDate(taskData.getDate()+7);
                                        }else if(list[i]["REPEATTERM"]=='1'){
                                            taskData.setDate(taskData.getDate()+1);
                                        }
                                    }
                                    // if(x>=s){
                                    //     $(this).addClass("nima");

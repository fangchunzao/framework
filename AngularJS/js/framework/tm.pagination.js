...
angular.module('tm.pagination', []).directive('tmPagination',[function(){
    return {
        ...
        template: '<div class="page-list">' +
            ...
            '<li ng-class="{dsabled: conf.currentPage == 1}" ng-click="prevPage()"><span>&laquo;</span></li>' +
            '<li ng-repeat="item in pageList track by $index" ng-class="{active: item == conf.currentPage, separate: item == \'...\'}" ' +
            'ng-click="changeCurrentPage(item)">' +
            '<span>{{ item }}</span>' +
            '</li>' +
            '<li ng-class="{dsabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()"><span>&raquo;</span></li>' +
            '</ul>' +
          ...
          /*  '跳转至<input type="text" ng-model="jumpPageNum" ng-keyup="jumpPageKeyUp($event)"/>' +
            '</div>' +*/
            '<div id="tip" class="no-items" ng-show="conf.totalItems <= 0" style="display:none">无符合条件数据</div>' +
            '</div>',
        replace: true,
        scope: {
            conf: '='
        ...
        link: function(scope, element, attrs) {
            ...
            scope.showDataList = function () {
                if (conf.totalInfoList == null || conf.totalInfoList == undefined) {
                    return ;
                }

                var currentPage = conf.currentPage;
                var itemsPerPage = conf.itemsPerPage;

                conf.showInfoList = conf.totalInfoList.slice(((currentPage - 1) * itemsPerPage),(currentPage * itemsPerPage));
            };
